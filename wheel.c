...
char **prune_word_list(char **words, int len, int *words_remaining) {
  ...
  while (ptr[i] != NULL) {
    if (strlen(ptr[i]) == len){
      //if list empty, malloc space for one char*
      if(prunelen == 0){
      ...
      // if non empty, realloc space for one more char*.
      else{
        newsize = sizeof(char*)*(prunelen+1);
        newlist = realloc(newlist, newsize);
        if (newlist == NULL) {
          perror("realloc");
          exit(1);
        }
      }
      newlist[prunelen] = ptr[i];
      prunelen++;
    }
    i++;
  }

  //didn't find word of length len.
  if (prunelen == 0) {
    newlist = malloc(sizeof(char*));
    if (newlist == NULL) {
      perror("malloc");
  ...
  //found at least one word of length len.
  else{
    newsize = sizeof(char*)*(prunelen+1);
    newlist = realloc(newlist, newsize);
    if (newlist == NULL) {
  ...
  newlist[prunelen] = NULL;
  *words_remaining = prunelen;
  return newlist;
}
...
char **get_word_list_of_length(char **words, int *len) {
  ...
  int pruneCond = 1;
  int words_remaining;
  while ((wordlen < 1 || wordlen > MAX_WORD_LENGTH) || pruneCond) {
    printf("Length of words to use? ");
    ...
    char *ptr;
    wordlen = strtol(input_buffer,&ptr, 10);
    //if there is some non-numeric char.
    if (*ptr != '\n') {
      printf("There are no words of that length.\n");
    }
    else{
      pruned = prune_word_list(words,wordlen,&words_remaining);
      pruneCond = 0;
      if(pruned[0] == NULL){
        printf("There are no words of that length.\n");
        pruneCond = 1;
  ...
  // wordlen should be a valid length when reach here.
  *len = wordlen;
  return pruned;
}
